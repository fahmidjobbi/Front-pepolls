              ...
              .then(recommendationData => {
                return fetch('http://127.0.0.1:5000/api/v1/auth/recommended-posts', {
                  ...
                  .then(recommendedPosts => {
                    ...
                    localStorage.setItem('user_interests', 'userData');
  ...
  const handleDeletePub = (pubId) => {
    ...
    .then(response => {
        if (response.ok) {
            ...
            addToast('Poll deleted', { appearance: 'success', autoDismiss: true,
...
const handleSharePub = (pubId) => {
  ...
  return (
    <div>
      ...
      {Array.isArray(pubs) && pubs.map(pub => (
        <Form marginTop={40}  width={680} key={pub.pub_id}>
